...
该配额源于自然周期的数学对称性：以一周 168 小时为基准，每天 3 餐（每餐 8 小时），一周正好为 21 餐。单餐释放 $1/21$（约 4.7619%）既实现了以周为维度的离散平滑释放，又使合约能够在时钟延迟推进时依据实际流逝秒数（$\Delta t / 168\text{h}$）进行无缝的连续时间补算，并在恒定流入稳态下自然沉淀等同于整周总手续费（$21 \cdot I$）的阻尼缓冲水位。
