...
- **单次写入永久 Spender**：授权提款地址（[`ExecutionRouter`]({% link zh/contracts.md %})）在合约生命周期中只能被绑定一次。一旦激活即永久生效且无法替换或升级（再次调用直接触发 `SpenderAlreadySet`）。
- **金库提款 7 天强制时间锁（`activationDelay = 604,800s`）**：合约部署后，治理方发起 `proposeSpender(router)` 提案，必须等待整整 7 天物理墙钟延迟方可调用 `activateSpender()`。在创世首周，The Belly 没有生效的 Spender，任何人都无法从中调拨资金。
- **协同运作的时钟闸门（`LaunchIntervalController`）**：与此同时，协议生产时钟硬编码将分红开启时间锁定在 `rewardStartAt = block.timestamp + 7 days`，在此期间分红计算强制为 0，允许早期质押者在公平起跑线上建立资历。
- **蓄水池深厚沉淀**：在这 7 天期间，PancakeSwap 交易产生的 4% 交易税通过 `flush()` 持续不断注入 The Belly“只进不出”，在正式开启常态化分红前沉淀出深厚的初始缓冲蓄水池。
