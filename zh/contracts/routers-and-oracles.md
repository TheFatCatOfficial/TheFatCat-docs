...
- **核心不变量**：
  - **单次写入永久 Spender**：在 The Belly 中，授权提款地址（`spender`）只能被设置并激活一次。一旦激活即永久生效且不可替换升级（再次调用直接 Revert `SpenderAlreadySet`）。7 天时间延迟为上线时的一次性激活时间锁（`activationDelay`），杜绝金库被任意重定向。
