...
欢迎查阅 **TheFatCat** 官方开发者与协议技术文档 —— 这是一个部署在 BNB Chain 上的去中心化交易税路由协议，基于物理水力阻尼模型构建，并搭载具有数学闭式解（Closed-Form $O(1)$）的资历阶梯加权算法。
...
1. **The Belly（大肚皮水力阻尼蓄水池）**：交易税资金进入一个无管理员特权的蓄水金库。它模拟一阶指数阻尼物理系统，并不即时倾倒给质押者，而是以每餐次（8小时）$\alpha = 8/168 \approx 4.7619\%$ 的阻尼速率平滑释放，将短期的剧烈市场波动转化为平稳、持久的资产库存。
...
- [背景与设计哲学]({% link zh/background.md %})：从直通式管道走向水力阻尼蓄水池。
...
协议底层数学模型与合约技术实现：
- [The Belly 水力阻尼机制]({% link zh/protocol/belly.md %})：离散释放公式、半衰期衰减与 7-窗口安全限额。
