...
The current **predeployment candidate** caps each conversion at **50 QQQB per swap**. This does not cap how much QQQB tax the Vault may receive, and there is no hourly conversion quota. The website reads the current on-chain cap and submits one batch of `min(Vault QQQB balance, per-swap cap)` for either SWAP or FEED; it does not offer a custom amount or automatically send multiple transactions. If 120 QQQB is pending, three successful calls would convert 50, 50 and 20 QQQB. After the first call, 70 QQQB remains in the Vault for later calls, each requiring a fresh check and its own gas payment. A direct contract caller may request a smaller permitted amount, but a request above the cap reverts instead of being silently reduced to 50. Every batch retains the Vault's mandatory price protection.
